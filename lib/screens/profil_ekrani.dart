...
class _ProfilEkraniState extends State<ProfilEkrani> {
    ...
    try {
      ...
      if (doc.exists && mounted) {
        ...
        if (avatar.startsWith("assets/") &&
            !avatar.startsWith("assets/image/")) {
  ...
  Future<void> _profiliDuzenle() async {
    final TextEditingController nameController = TextEditingController(
      text: _user?.displayName ?? widget.kullaniciAdi,
    );
                ...
                Row(
                  children: [
                    _yasSecenek(
                      "6-12",
                      tempYas,
                      (val) => setDialogState(() => tempYas = val),
                    ),
      ...
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(
              color: isSelected ? AppColors.anaMavi : Colors.grey.shade300,
            ),
          ),
          child: Text(
            label,
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isSelected ? AppColors.anaMavi : Colors.transparent,
            width: 3,
          ),
        ),
  ...
  Future<void> _cikisYap() async {
    bool? onay = await _onayDiyalogu(
      ...
      icerik:
          "Kahramanlık görevine ara mı vermek istiyorsun? Seni özleyeceğiz!",
  ...
  Future<void> _hesabiSil() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        _showSnack("Function hatası: ${e.code} - ${e.message}", isError: true);
      }
    } catch (e, stack) {
      ...
      if (mounted) {
        ...
        _showSnack("Beklenmeyen hata: $e", isError: true);
      }
    }
  }
  ...
  Future<Map<String, dynamic>> _getStats(String uid) async {
    try {
      final userDoc = await FirebaseFirestore.instance
          ...
          .doc(uid)
          .get();
      bool admin = userDoc.exists
          ? (userDoc.data()?['isAdmin'] ?? false)
          : false;
      final sCountQuery = await FirebaseFirestore.instance
          .collection('scenarios')
          .count()
          .get();
      final hCountQuery = await FirebaseFirestore.instance
          .collection('stories')
          .count()
          .get();
      final dCountQuery = await FirebaseFirestore.instance
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: _isDeleting
          ? const Center(
              child: CircularProgressIndicator(color: AppColors.anaMavi),
            )
          : StreamBuilder<DocumentSnapshot>(
              ...
              builder: (context, snapshot) {
                ...
                final List bitti =
                    progressData['tamamlanan_bolumler'] as List? ?? [];
                final List okundu =
                    progressData['okunan_hikayeler'] as List? ?? [];
                final List dedektif =
                    progressData['bilinen_dedektif_sorulari'] as List? ?? [];
                ...
                return FutureBuilder<Map<String, dynamic>>(
                  ...
                  builder: (context, statsSnap) {
                    ...
                    final double ilerleme = (tamamlananToplam / toplamGorev)
                        .clamp(0.0, 1.0);
              ...
              Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 12),
                    Text(
                      "Harika ilerliyorsun, süper kahraman! 🚀",
                      style: TextStyle(
    ...
    return Padding(
      ...
      child: InkWell(
        onTap: () => Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => AdminGuard(child: const AdminHome()),
          ),
        ),
        child: Container(
  ...
  Widget _buildActionGrid(Size size) {
    final items = [
      _AksiyonOge(
        ...
        onTap: () => Navigator.push(
          context,
          MaterialPageRoute(builder: (_) => const GuvenlikRehberiEkrani()),
        ),
      ),
      _AksiyonOge(
        ...
        child: Column(
          ...
          children: [
            Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(
                color: oge.color.withOpacity(0.1),
                shape: BoxShape.circle,
              ),
              child: Icon(oge.icon, color: oge.color, size: 22),
            ),
            const SizedBox(height: 8),
            Text(
              oge.label,
  ...
  Widget _buildFooter() {
    return Column(
      children: [
        ...
        const Text(
          "TÜBİTAK Akran Zorbalığı Farkındalık Projesi",
  ...
  Future<void> _bizeUlasin() async {
    ...
    final Uri emailLaunchUri = Uri(
      ...
      query: 'subject=Kahraman Geri Bildirim | $isim',
    );
    try {
