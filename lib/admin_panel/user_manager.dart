...
class _UserManagerState extends State<UserManager> {
              ...
              child: StreamBuilder<QuerySnapshot>(
                ...
                builder: (context, snapshot) {
                  ...
                  if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
                    return const Center(child: Text("Sistemde henüz bir kahraman bulunmuyor."));
                  }
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return _buildUserCard(username, yasGrubu, uid, isOnline, sonGorulme);
                    },
                  );
                },
              ),
  ...
  Widget _buildUserCard(String username, String yas, String uid, bool isOnline, Timestamp? sonGorulme) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
              color: isOnline ? Colors.green.withOpacity(0.08) : Colors.black.withOpacity(0.02),
              blurRadius: 20,
              offset: const Offset(0, 10)
          )
        ],
        border: Border.all(color: isOnline ? Colors.green.withOpacity(0.2) : Colors.transparent, width: 1.5),
      ),
      child: ListTile(
          ...
          children: [
            Container(
              decoration: BoxDecoration(
                ...
                boxShadow: [if (isOnline) BoxShadow(color: Colors.green.withOpacity(0.3), blurRadius: 15, spreadRadius: 2)],
              ),
              child: CircleAvatar(
                radius: 32,
        ...
        title: Row(
          children: [
            Text(username, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 19, color: Color(0xFF1E293B))),
            const SizedBox(width: 10),
            if (isOnline)
              _statusBadge("CANLI", Colors.green),
          ],
        ),
        subtitle: Padding(
          ...
          child: Row(
            children: [
              ...
              Icon(Icons.history_toggle_off_rounded, size: 14, color: Colors.blueGrey.shade300),
                          ...
                          children: [
                            ...
                            Row(
                              children: [
                                ...
                                _premiumStatBox("Eğitim Süresi", "${stats['toplam_sure_dk'] ?? 0} dk", Icons.timer_rounded, Colors.blue),
                                const SizedBox(width: 20),
                                _premiumStatBox("Hatalı Karar", "${stats['hatali_cevaplar'] ?? 0}", Icons.error_outline_rounded, Colors.red),
                              ],
                            ),
  ...
  Widget _buildCharacterChart(Map karar) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(30), border: Border.all(color: const Color(0xFFF1F5F9))),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text("%${(val * 100).toInt()}", style: TextStyle(color: color, fontWeight: FontWeight.w900, fontSize: 13)),
            ],
          ),
  ...
  Widget _buildBadgeAndMissionList(List rozetler, List bolumler) {
    return Column(
      children: [
        _infoSection("KAZANILAN ROZETLER", Icons.verified_rounded, rozetler, Colors.amber),
        const SizedBox(height: 20),
        _infoSection("TAMAMLANAN GÖREVLER", Icons.task_alt_rounded, bolumler, Colors.green),
      ],
    );
  }
  ...
  Widget _infoSection(String title, IconData icon, List items, Color color) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: [Icon(icon, size: 16, color: color), const SizedBox(width: 8), Text(title, style: TextStyle(fontWeight: FontWeight.w900, fontSize: 12, color: color, letterSpacing: 1))]),
          const SizedBox(height: 15),
          items.isEmpty
              ? Text("Henüz veri yok.", style: TextStyle(color: Colors.grey.shade400, fontSize: 11, fontStyle: FontStyle.italic))
              : Wrap(spacing: 8, runSpacing: 8, children: items.map((i) => _badge(i.toString(), color)).toList()),
        ],
      ),
    );
  }
  ...
  Widget _premiumStatBox(String t, String v, IconData i, Color c) {
    return Expanded(
      child: Container(
        ...
        child: Column(
          children: [
            Container(padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: c.withOpacity(0.1), shape: BoxShape.circle), child: Icon(i, color: c, size: 24)),
            const SizedBox(height: 12),
            Text(v, style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 22, color: Color(0xFF1E293B))),
            Text(t, style: TextStyle(color: Colors.blueGrey.shade300, fontSize: 11, fontWeight: FontWeight.bold)),
          ],
        ),
      ),
    );
  ...
  Widget _actionIconButton(IconData i, Color c, VoidCallback t) {
    return Container(
      decoration: BoxDecoration(color: c.withOpacity(0.08), borderRadius: BorderRadius.circular(15)),
      child: IconButton(onPressed: t, icon: Icon(i, color: c, size: 22), hoverColor: c.withOpacity(0.15)),
    );
  }
  ...
  Widget _statusBadge(String t, Color c) {
    return Container(
      ...
      child: Text(t, style: TextStyle(color: c, fontWeight: FontWeight.w900, fontSize: 9, letterSpacing: 0.5)),
    );
  }
  ...
  Widget _infoBadge(String t, Color c) {
    return Container(
      ...
      child: Text(t, style: const TextStyle(color: Color(0xFF64748B), fontWeight: FontWeight.bold, fontSize: 11)),
    );
  }
  ...
  Widget _badge(String t, Color c) {
    return Container(
      ...
      child: Text(t.toUpperCase(), style: TextStyle(color: c, fontWeight: FontWeight.w900, fontSize: 10)),
    );
  }
  ...
  Widget _buildTotalUserBadge() {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, s) => Container(
        ...
        child: Text("Toplam ${s.hasData ? s.data!.docs.length : 0} Kahraman",
    ...
    return Row(
      ...
      children: [
        Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(u.toUpperCase(), style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: Color(0xFF1E293B))),
          const Text("Detaylı Performans Analizi", style: TextStyle(color: Colors.grey, fontSize: 13)),
        ]),
        const CloseButton(),
      ],
    );
  ...
  void _deleteUser(String username, String uid) {
    showDialog(context: context, builder: (c) => AlertDialog(
      ...
      content: Text("$username kullanıcısını ve tüm verilerini sistemden silmek istediğinize emin misiniz?"),
      actions: [
